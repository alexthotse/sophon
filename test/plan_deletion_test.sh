# Sophon hasn't been able to get this working yet
...
count_plans() {
    ...
    sdxd plans | sed 's/\x1B\[[0-9;]*[mK]//g' | grep -A 1000 "^+----+" | tail -n +3 | head -n 5 | grep -E '\| (plan-config-[123]|other-plan-[12])(\.[0-9]+)? \|' | wc -l
}

# Clean up any existing test plans before starting
echo "Cleaning up any existing test plans..."
echo "y" | sdxd dp "plan-config-*" || true
echo "y" | sdxd dp "other-plan-*" || true
sleep 2
...
echo "y" | sdxd dp "plan-config-*" || true
echo "y" | sdxd dp "other-plan-*" || true
echo "y" | sdxd dp "range-test-*" || true
sleep 2
