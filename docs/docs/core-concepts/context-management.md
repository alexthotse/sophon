...
Context in Sophon refers to files, directories, URLs, images, notes, or piped in data that the LLM uses to understand and work on your project. Context is always associated with a [plan](./plans.md).

Changes to context are [version controlled](./version-control.md) and can be [branched](./branches.md).

## Automatic vs. Manual

As of v2, Sophon loads context automatically by default. When a new plan is created, a [project map](#loading-project-maps) is generated and loaded into context. The LLM then uses this map to select relevant context before planning a task or responding to a message.

### Tradeoffs

Automatic context loading makes Sophon more powerful and easier to work with, but there are tradeoffs in terms of cost, focus, and output speed. If you're trying to minimize costs or you know for sure that only one or two files are relevant to a task, you might prefer to load context manually.
...
```bash
sophon set-auto plus
sophon set-auto basic
sophon set-auto default plus # set the default value for all new plans
```
...
```bash
sophon set-config auto-load-context false
sophon set-config default auto-load-context false # set the default value for all new plans
```

### Smart Context Window Management

Another new context management feature in v2 is smart context window management. When making a plan with multiple steps, Sophon will determine which files are relevant to each step. Only those files will be loaded into context during implementation.
...
```bash
sophon set-config smart-context true
sophon set-config smart-context false
sophon set-config default smart-context false # set the default value for all new plans
```

### Automatic Context Updates

When you make your own changes to files in context separately from Sophon, those files need to be updated before the plan can continue. Previously, Sophon would prompt you to update context every time a file was changed. This is now automatic by default.

Automatic updates are enabled in the `plus` autonomy level and above. You can also toggle them with `set-config`:

```bash
sophon set-config auto-update-context true
sophon set-config auto-update-context false
sophon set-config default auto-update-context false # set the default value for all new plans
```
...
Another use for manual context loading is for context types that can't be loaded automatically, like URLs, notes, or piped data (for now Sophon can only automatically load project files and images within the project).

## Manually Loading Context

To load files, directories, directory layouts, urls, images, notes, or piped data into a plan's context, use the `sophon load` command.
...
sophon load * # loads all files in the current directory
```

You can load context from parent or sibling directories if needed by using `..` in your load paths.

```bash
sophon load ../file.go # loads file.go from parent directory
sophon load ../sibling-dir/test.go # loads test.go from sibling directory
```
...
```bash
sophon load lib -r # loads lib, all its files and all its subdirectories
sophon load * -r # loads all files in the current directory and all its subdirectories
```

### Loading Files and Directories in the REPL

In the [Sophon REPL](../repl.md), you can use the shortcut `@` plus a relative file path to load a file or directory into context.
...
sophon load src/components --tree # loads the layout of the src/components directory
```

### Loading Project Maps

Sophon can create a **project map** for any directory using [tree-sitter](https://tree-sitter.github.io/tree-sitter). This shows all the top-level symbols, like variables, functions, classes, etc. in each file. 30+ languages are supported. For non-supported languages, files are still listed without symbols so that the model is aware of their existence.
...
Sophon can load the text content of URLs, which can be useful for adding relevant documentation, blog posts, discussions, and the like.
...
Sophon can load images into context.

```bash
sophon load ui-mockup.png
```
...
If you're in a git repo, Sophon respects `.gitignore` and won't load any files that you're ignoring. You can also add a `.sophonignore` file with ignore patterns to any directory.

You can force Sophon to load ignored files with the `--force/-f` flag:

```bash
sophon load .env --force # loads the .env file even if it's in .gitignore or .sophonignore
```

## Viewing Context

To list everything in context, use the `sophon ls` command:

```bash
sophon ls
```

You can also see the content of any context item with the `sophon show` command:

```bash
sophon show component.ts # show the content of component.ts
sophon show 2 # show the content of the second item in the `sophon ls` list
```

## Removing Context

To remove selectively remove context, use the `sophon rm` command:

```bash
sophon rm component.ts # remove by name
sophon rm 2 # remove by number in the `sophon ls` list
sophon rm 2-5 # remove a range of indices
sophon rm lib/**/*.js # remove by glob pattern
sophon rm lib # remove whole directory
```

## Clearing Context

To clear all context, use the `sophon clear` command:
...
If files, directory layouts, or URLs in context are modified outside of Sophon, they will need to be updated next time you send a prompt.
...
```bash
sophon update # update files in context
```
