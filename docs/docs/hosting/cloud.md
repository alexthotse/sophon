...
- Use Sophon credits to pay for AI models.
...
[Get started with Integrated Models Mode.](https://app.sophon.ai/start?modelsMode=integrated)
...
[Get started with BYO API Key Mode.](https://app.sophon.ai/start?modelsMode=byo)

## Billing Settings

Run `sophon billing` in the terminal to bring up the billing settings page in your default browser, or go to [your Billing Settings page](https://app.sophon.ai/settings/billing) (sign in if necessary).
...
You can see your current balance and a report on recent usage with `sophon usage` (`\usage` in the REPL):

```bash
sophon usage
```

You can see a log of individual transactions that includes every model call with `sophon usage --log`:

```bash
sophon usage --log
```

In the Sophon REPL, `usage` defaults to showing usage for the current REPL session. Otherwise, it defaults to showing usage for the day so far.
...
```bash
sophon usage --today # show usage for the day so far
sophon usage --month # show usage for the current billing month
sophon usage --plan # show usage for the current plan
```
...
```bash
sophon usage --log --debits --month # show only debits for the current billing month
sophon usage --log --purchases --today # show only purchases for the day so far
```
...
- Your name and email is shared with [Loops](https://loops.so/), an email marketing service, in order to send you updates on Sophon. You can opt out of these emails at any time with one click.
...
- [Relace](https://relace.ai/) for an instant apply AI model that speeds up and reduces the cost of file edits. Used as a fallback if Sophon is unable to apply edits deterministically. Inputs are the original file and the edit snippet from a Sophon response.
...
See our full [Privacy Policy](https://sophon.ai/privacy) for more details.
