...
- Now when using Sophon in BYO key mode (either Cloud or self-hosted), you can easily use Sophon with a wide range of built-in model providers.
...
- Sophon now supports JSON-based model config, which make it much easier to try out different models, different settings, and to use custom models, providers, and model packs.

- The new JSON model config system integrates cleanly with your IDE or editor. When you first edit model settings, Sophon will prompt you to set a preferred editor, and the JSON config file will be opened in that editor.
...
- Check out the new docs for [model settings](https://docs.sophon.ai/models/model-settings) and [custom models](https://docs.sophon.ai/models/custom-models) for full details.
...
- The docs now include all built-in [models](https://docs.sophon.ai/models/built-in/built-in-models) and [model packs](https://docs.sophon.ai/models/built-in/built-in-packs), making it easier to see what's available, and the settings for each model/model pack.
...
- `sophon checkout` now has a `--yes/-y` flag to auto-confirm creating a new branch if it doesn't exist, so the command can be used for scripting with no user interaction.

- `sophon tell`, `sophon continue`, and `sophon build` all now support a `--skip-menu` flag to skip the interactive menu that appears when the response finishes and changes are pending. There's also a new `skip-changes-menu` config setting that can be set to `true` to skip this menu by default.
...
- When using a BYO key mode (either cloud or self-hosted), you can now use Sophon with **only** an OpenRouter.ai account and `OPENROUTER_API_KEY` set. A separate OpenAI account is no longer required.
...
- A new `gemini-preview` model pack has been added, which uses Gemini 2.5 Pro Preview for planning and coding, and default models for other roles. You can use this pack by running the REPL with the `--gemini-preview` flag (`sophon --gemini-preview`), or with `\set-model gemini-preview` from inside the REPL. Because this model is still in preview, a fallback to Gemini 1.5 Pro is used on failure.
...
- OpenAI's o4-mini is now available as a built-in model with `high`, `medium`, and `low` reasoning effort levels. o3-mini has been replaced by the corresponding o4-mini models across all model packs, with a fallback to o3-mini on failure. This improves Sophon's file edit reliability and performance with no increase in costs. o4-mini-medium is also the new default planning model for the `cheap` model pack.

- OpenAI's o3 is now available as a built-in model with `high`, `medium`, and `low` reasoning effort levels. Note that if you're using Sophon in BYO key mode, OpenAI requires an organization verification step before you can use o3.
...
## CLI Version 2.0.5
- Consolidated to a single model pack for Gemini 2.5 Pro Experimental: 'gemini-exp'. Use it with 'sophon --gemini-exp' or '\set-model gemini-exp' in the REPL.
...
- **Debugging**
  - Sophon can now directly debug browser applications by catching errors and reading the console logs (requires Chrome).
...
I'm excited to announce the beta release of Sophon v2, featuring major improvements in capabilities, user experience, and automation.
...
Sophon is now a top-tier coding agent with fully autonomous capabilities.
...
- v2 includes a new default way to use Sophon: the Sophon REPL. Just type `sophon` in any project directory to start the REPL.
...
- Sophon is now capable of full autonomy with 'full auto' mode. It can load necessary context, apply changes, execute commands, and automatically debug problems.

- The automation level can be precisely configured depending on the task and your comfort level. A `basic` mode works just like Sophon v1, where files are loaded manually and execution is disabled. The new default in v2 is `semi-auto`, which enables automatic context loading, but still requires approval to apply changes and execute commands.

- By default, Sophon now includes command execution (with approval) in its planning process. It can install dependencies, build and run code, run tests, and more.

- Command execution is integrated with Sophon's diff review sandbox. Changes are tentatively applied before running commands, then rolled back if the command fails.
...
This isn't an exhaustive list! Apart from the above, there are many smaller features, bug fixes, and quality of life improvements. Give the updated [docs](https://docs.sophon.ai) a read for a full accounting of all commands and functionality.

## 🌟  Get Started

Go to the [quickstart](https://docs.sophon.ai/quickstart) to get started with v2 in minutes.

**Note:** while built on the same foundations, Sophon v2 is designed to be a run separately and independently from v1. It's not an in-place upgrade. So there's nothing in particular you need to do to upgrade; just follow the quickstart as if you were a brand new user. [More details here.](https://docs.sophon.ai/upgrading-v1-to-v2)
...
- You can follow [@SophonAI](https://x.com/sophonai) or my personal account [@Danenania](https://x.com/danenania) on X for updates, announcements, and random musings.

- You can subscribe on [YouTube](https://www.youtube.com/@sophon-ny5ry) for demonstrations, tutorials, and AI coding projects.
...
You can now easily use Claude 3.5 Sonnet with Sophon through OpenRouter.ai.
...
- Sophon is much less likely to screw up braces, parentheses, and other code structures.
- Sophon is much less likely to mistakenly remove code that it shouldn't.

## Major improvements to long plans with many steps 🛤️
- Sophon's 'working memory' has been upgraded. It is now much better at working through very long plans without skipping tasks, repeating tasks it's already done, or otherwise losing track of what it's doing.
...
- Use `sophon plans --archived` (or `sophon plans -a`) to see archived plans in the current directory.
- Use `sophon unarchive` (or `sophon unarc`) to restore an archived plan.
...
- Some built-in models and model packs (see 'Model packs' below) have been included as a quick way to try out a few of the more powerful model options. Just use `sophon set-model` to try these.
- You can use a custom model you've added with `sophon set-model`, or add it to a model pack (see 'Model packs' below) with `sophon model-packs create`. Delete custom models you've added with `sophon models delete`.
...
- Use `sophon model-packs create` qto create your own model packs.
- Use `sophon model-packs` to list built-in and custom model packs.
- Use `sophon set-model` to load a model pack.
- Use `sophon model-packs delete` to remove a custom model pack.
...
- Use `sophon models default` to see default model settings and `sophon set-model default` to update them.

## More commands 💻
- `sophon summary` to see the latest plan summary
- `sophon rename` to rename the current plan
