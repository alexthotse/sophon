...
func TestBufferOrStream(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			initialState: &chunkProcessor{
				fileOpen:                true,
				contentBuffer:           `<SophonBlock lang="go" path="main.go">` + "\n",
				awaitingBlockOpeningTag: true,
			},
			chunk:           `package`,
			maybeFilePath:   "",
		...
		},
		{
			...
			chunk:           `<SophonBlock lang="go" path="main.go">` + "\npackage",
			maybeFilePath:   "",
			currentFilePath: "main.go",
			want: bufferOrStreamResult{
				shouldStream: true,
		...
		},
		{
			...
			wantState: &chunkProcessor{
				awaitingBlockOpeningTag: true,
				contentBuffer:           "something<SophonBlock lang=\"go\" path=\"main",
			},
		},
		{
...

		{
			...
			chunk:        "hello <SophonFinish/>bye",
			manualStop:   []string{"<SophonFinish/>"},
			want: bufferOrStreamResult{
		...
		},
		{
			...
			chunk:      "<SophonFin", // no '<' in second part
			manualStop: []string{"<SophonFinish/>"},
			want: bufferOrStreamResult{
			...
			wantState: &chunkProcessor{
				contentBuffer: "<SophonFin", // prefix is buffered
			},
		},
		{
			...
			initialState: &chunkProcessor{
				contentBuffer: "<SophonFin", // leftover from previous call
			},
