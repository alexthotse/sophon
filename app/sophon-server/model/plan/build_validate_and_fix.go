...
import (
	...
	"log"
	"math/rand"
	diff_pkg "sophon-server/diff"
	"sophon-server/model"
	"sophon-server/model/prompts"
...
func (fileState *activeBuildStreamFileState) buildValidate(
	...
	stop := []string{"<SophonFinish/>"}
	if params.validateOnly {
		stop = []string{"<SophonComments>", "<SophonReplacements>"}
	}
...
func handleXMLResponse(
	...
	if strings.Contains(content, "<SophonCorrect/>") {
	...
	replacementsOuter := utils.GetXMLContent(content, "SophonReplacements")
	...
	for i, replacement := range replacements {
		...
		firstLineNum, err := shared.ExtractLineNumberWithPrefix(firstLine, "sdx-")
		if err != nil {
		...
		if lastLine != "" {
			lastLineNum, err = shared.ExtractLineNumberWithPrefix(lastLine, "sdx-")
			if err != nil {
	...
	if strings.Contains(content, "<SophonIncorrect/>") {
		split := strings.Split(content, "<SophonIncorrect/>")
		problem = split[0]
	} else if strings.Contains(content, "<SophonReplacements>") {
		split := strings.Split(content, "<SophonReplacements>")
		problem = split[0]
	}
