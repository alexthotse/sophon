...
var openingTagRegex = regexp.MustCompile(`<SophonBlock\s+lang="(.+?)"\s+path="(.+?)".*?>`)
...
func (state *activeTellStreamState) processChunk(choice types.ExtendedChatCompletionStreamChoice) processChunkResult {
	...
	if processor.fileOpen && strings.HasSuffix(active.CurrentReplyContent+content, "</SophonBlock>") {
		if verboseLogging {
...
func (processor *chunkProcessor) bufferOrStream(content string, parserRes *types.ReplyParserRes, currentStage shared.CurrentStage, manualStopSequences []string) bufferOrStreamResult {
	...
	if awaitingTag {
		...
		if processor.awaitingBlockOpeningTag {
			...
			if parserRes.CurrentFilePath != "" {
				...
				if matched {
					...
					if verboseLogging {
						log.Printf("Opening <SophonBlock> tag is missing even though parserRes.CurrentFile is set - something is wrong: %s\n", content)
					}
					processor.awaitingBlockOpeningTag = false
					processor.fileOpen = false
				...
				if len(split) > 1 {
					...
					if strings.HasPrefix(`SophonBlock lang="`, last) {
						if verboseLogging {
							log.Println("strings.HasPrefix(`SophonBlock lang=", last)
						}
						shouldStream = false
						matchedPrefix = true
					} else if strings.HasPrefix(last, `SophonBlock lang="`) {
						if verboseLogging {
			...
			if parserRes.CurrentFilePath == "" {
				if strings.Contains(content, "</SophonBlock>") {
					shouldStream = true
					processor.awaitingBlockClosingTag = false
					processor.fileOpen = false
					// replace </SophonBlock> with ``` to close the markdown code block
					content = strings.ReplaceAll(content, "</SophonBlock>", "```")
				} else {
					log.Printf("Closing </SophonBlock> tag is missing even though parserRes.CurrentOperation is nil - something is wrong: %s\n", content)
					processor.awaitingBlockClosingTag = false
					shouldStream = true
				}
			}
			...
			if strings.Contains(content, "<EndSophonFileOps/>") {
				if verboseLogging {
					log.Printf("Found <EndSophonFileOps/>\n")
				}
				processor.awaitingOpClosingTag = false
				content = strings.Replace(content, "\n<EndSophonFileOps/>", "", 1)
				content = strings.Replace(content, "<EndSophonFileOps/>", "", 1)
				shouldStream = true
			}
		}
		...
		if parserRes.MaybeFilePath != "" && parserRes.CurrentFilePath == "" {
			...
			// this will set processor.awaitingBlockOpeningTag to true if the content starts with any prefix of<SophonBlock lang=" *or* any prefix of a full opening tag
		...
		if parserRes.CurrentFilePath != "" {
			...
			if strings.Contains(content, "</SophonBlock>") {
				if verboseLogging {
					log.Println("strings.Contains(content, \"</SophonBlock>\")")
				}
				processor.awaitingBlockClosingTag = true
			} else {
				if verboseLogging {
					log.Println("not strings.Contains(content, \"</SophonBlock>\")")
				}
				split := strings.Split(content, "<")
				// log.Printf("split: %v\n", split)
				if len(split) > 1 {
					...
					last := split[len(split)-1]
					// log.Printf("last: %s\n", last)
					if strings.HasPrefix("/SophonBlock>", last) {
						if verboseLogging {
							log.Println("strings.HasPrefix(\"/SophonBlock>\", last)")
						}
						processor.awaitingBlockClosingTag = true
					}
				}
			...
			if strings.Contains(content, "<EndSophonFileOps/>") {
				if verboseLogging {
					log.Println("strings.Contains(content, \"<EndSophonFileOps/>\")")
				}
				processor.awaitingOpClosingTag = true
			} else {
				if verboseLogging {
					log.Println("not strings.Contains(content, \"<EndSophonFileOps/>\")")
				}
				split := strings.Split(content, "<")
				if len(split) > 1 {
					if verboseLogging {
					...
					last := split[len(split)-1]
					if strings.HasPrefix("EndSophonFileOps/>", last) {
						if verboseLogging {
							log.Println("strings.HasPrefix(\"EndSophonFileOps/>\", last)")
						}
						processor.awaitingOpClosingTag = true
					}
				}
			}
		} else if strings.Contains(content, "</SophonBlock>") {
			if verboseLogging {
				log.Println("strings.Contains(content, \"</SophonBlock>\")")
			}
			content = strings.Replace(content, "</SophonBlock>", "```", 1)
		} else if strings.Contains(content, "<EndSophonFileOps/>") {
			if verboseLogging {
				log.Println("strings.Contains(content, \"<EndSophonFileOps/>\")")
			}
			content = strings.Replace(content, "\n<EndSophonFileOps/>", "", 1)
...
func replaceCodeBlockOpeningTag(content string, replaceWithFn func(lang string) string) (bool, string) {
	// check for opening tag matching <SophonBlock lang="..." path="...">
	...
	if match != nil {
		...
		return true, strings.Replace(content, "<SophonBlock>", replaceWithFn(""), 1)
	}
