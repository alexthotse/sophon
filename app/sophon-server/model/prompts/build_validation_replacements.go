...
Line numbers prefixed with 'sdx-' are included in the original file. Line numbers prefixed with 'sdx-new-' are included in the proposed changes. The diff WILL NOT include these line numbers and you must not include them in your evaluation. You must ignore them completely.
...
If the changes were applied *correctly*, you MUST output a <SophonCorrect/> tag, followed by a <SophonFinish/> tag, then end your response, like this:
...
Next, you MUST output a <SophonIncorrect/> tag, and then proceed to output the <SophonComments/> tag and the <SophonReplacements/> tag with at least one <Replacement> element (see below for details). Example:
...
Next, if the changes were applied *incorrectly*, you must analyze the *original file* and the *proposed updates* and output a <SophonReplacements> element that applies the changes described in the *proposed updates* to the *original file* in order to produce a final, valid resulting file with all changes correctly applied.

CRITICALLY IMPORTANT: When applying changes with replacements, NO REFERENCE COMMENTS CAN BE PRESENT IN THE RESULTING FILE. All reference comments (as listed in the <SophonComments> element above) ABSOLUTELY MUST be replaced with the code they refer to in the *original file*.

Now output a <SophonReplacements> element that contains all the replacements needed to correctly apply the changes described in the *proposed updates* to the *original file*. The <SophonReplacements> element MUST contain at least one <Replacement> element.
...
The <Old> element must contain the *exact* original code that will be replaced. *Every* character in the <Old> element must be present in the original file. You MUST include line numbers prefixed with 'sdx-' in the <Old> element (NOT with 'sdx-new-'). Every line in the <Old> element must exactly match a line in the original file, including spacing, indentation, newlines, and the 'sdx-' line number. <Old> MUST NOT contain any partial lines, only complete lines.

The <New> element must contain ALL the new code that will replace the code in <Old>. It must contain complete lines only (no partial lines). It must be syntactically correct and valid for the given programming language. It MUST NOT contain any line numbers. It MUST NOT contain any reference comments listed in the <SophonComments> element. ALL reference comments ABSOLUTELY MUST be replaced with the actual code they refer to in the *original file*.
...
<SophonReplacements>
  <Replacement>
    <Old>
      sdx-42: func someFunction() {
      sdx-43:   connectToDatabase()
      sdx-44: }
    </Old>
    <New>
      func someFunction() {
...
IMPORTANT RULES:
1. If your evaluation finds ANY issues, you MUST use <SophonIncorrect/> followed by a <SophonComments> element and a <SophonReplacements> element with at least one <Replacement> element.
2. If your evaluation finds NO issues, you MUST use <SophonCorrect/> then a <SophonFinish/> element. Do NOT output comments or replacements if the changes were applied correctly.
3. In replacements, every line in the <Old> element MUST exactly match a line in the original file and MUST begin with the line number with a 'sdx-' prefix (NOT with a 'sdx-new-' prefix).
4. In replacements, lines in the <New> element MUST NOT begin with a line number or prefix.
5. Always include reasoning in a '## Evaluate Diff' section prior to outputting the <SophonCorrect/> or <SophonIncorrect/> tags.

--

DO NOT FORGET TO INCLUDE THE ***'sdx-' PREFIXED*** LINE NUMBERS IN THE <Old> ELEMENT.
`
...
Original file (with line nums prefixed with 'sdx-'):
>>>
%s
<<<
...
Proposed changes (with line nums prefixed with 'sdx-new-'):
>>>
%s
<<<
`,
