...
func (r *ReplyParser) AddChunk(chunk string, addToTotal bool) {
	...
	if r.maybeFilePath != "" && !r.isInMoveBlock && !r.isInRemoveBlock && !r.isInResetBlock {
		...
		if strings.HasPrefix(prevFullLineTrimmed, "<SophonBlock") {
			if verboseLogging {
	...
	if r.currentFilePath == "" && !r.isInMoveBlock && !r.isInRemoveBlock && !r.isInResetBlock {
		...
		if LineHasXmlPath(prevFullLineTrimmed) {
			if verboseLogging {
				log.Println("Line has XML-style SophonBlock tag")
			}
		...
		if prevFullLineTrimmed == "</SophonBlock>" {
			if verboseLogging {
		...
		if prevFullLineTrimmed == "<EndSophonFileOps/>" {
			if verboseLogging {
...
func LineHasXmlPath(line string) bool {
	return strings.HasPrefix(line, "<SophonBlock") && strings.Contains(line, `path="`)
}
...
func extractFilePath(line string) string {
	// Handle XML-style SophonBlock tag
	if strings.HasPrefix(line, "<SophonBlock") {
