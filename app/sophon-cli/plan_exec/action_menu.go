...
func handleHotkeyOption(option hotkeyOption, diffs []string, params ExecParams) {
	...
	if option.char == "d" {
		fmt.Println()
		_, err := lib.ExecSophonCommandWithParams([]string{"diffs", "--ui", "--from-tell-menu"}, lib.ExecSophonCommandParams{
		...
		fmt.Println()
		_, err := lib.ExecSophonCommandWithParams([]string{"diffs", "--git"}, lib.ExecSophonCommandParams{
			DisableSuggestions: true,
		})
		if err != nil {
			fmt.Printf("\nError showing diffs: %v\n", err)
		...
		fmt.Print("(a)")
		fmt.Println()
		_, err := lib.ExecSophonCommand([]string{"apply"})
		if err != nil {
		...
		fmt.Println()
		_, err := lib.ExecSophonCommand([]string{"reject"})
		if err != nil {
		...
		_, err := lib.ExecSophonCommand([]string{"apply", "--full"})
		if err != nil {
			fmt.Printf("\nError applying changes: %v\n", err)
		}
		fmt.Println()
