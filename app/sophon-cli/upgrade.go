...
func checkForUpgrade() {
	...
	defer cancel()
	latestVersionURL := "https://sophon.ai/v2/cli-version.txt"
	...
	if latestVersion.GreaterThan(currentVersion) {
		term.StopSpinner()
		fmt.Println("A new version of Sophon is available:", color.New(color.Bold, term.ColorHiGreen).Sprint(versionStr))
...
func doUpgrade(version string) error {
	...
	for {
		...
		if header.Typeflag == tar.TypeReg && (header.Name == "sophon" || header.Name == "sophon.exe") {
