...
func WriteCurrentPlan(id string) error {
	if fs.HomeSophonDir == "" {
		return fmt.Errorf("HomeSophonDir not set")
	}
...
func WriteCurrentBranch(branch string) error {
	...
	dir := filepath.Join(fs.HomeSophonDir, CurrentProjectId, CurrentPlanId)
...
func GetCurrentBranchNamesByPlanId(planIds []string) (map[string]string, error) {
	if fs.HomeSophonDir == "" {
		return nil, fmt.Errorf("HomeSophonDir not set")
	}
...
func getPlanCurrentBranch(planId string) (string, error) {
	...
	v2Path := filepath.Join(fs.HomeSophonDir, CurrentProjectId, planId, "settings-v2.json")
