...
func init() {
	ReplSettingsDir = filepath.Join(fs.HomeSophonDir, "repl_settings")
}
...
// ExecSophonCommand spawns the same binary, wiring std streams directly so you
// don't have to capture output. Any os.Exit calls in the child won't kill your REPL.
func ExecSophonCommand(args []string) (string, error) {
	return ExecSophonCommandWithParams(args, ExecSophonCommandParams{})
}

type ExecSophonCommandParams struct {
	DisableSuggestions bool
	SessionId          string
}

func ExecSophonCommandWithParams(args []string, params ExecSophonCommandParams) (string, error) {
	// Create temp file
	tmpFile, err := os.CreateTemp("", "sophon-output-*")
	if err != nil {
		return "", err
	}
