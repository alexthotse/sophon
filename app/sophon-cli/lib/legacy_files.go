...
func MigrateLegacyProjectFile(currentUserId string) {
	...
	// New formats map to a user id so we can handle multiple accounts in the same sophon dir
	projectPath := filepath.Join(fs.SophonDir, "project.json")
	if _, err := os.Stat(projectPath); err == nil {
		...
		v2Path := filepath.Join(fs.SophonDir, "projects-v2.json")
...
func MigrateLegacyCurrentPlanFile(currentUserId string) {
	...
	planPath := filepath.Join(fs.HomeSophonDir, CurrentProjectId, "current_plan.json")
	...
	if _, err := os.Stat(planPath); err == nil {
		...
		v2Path := filepath.Join(fs.HomeSophonDir, CurrentProjectId, "current-plans-v2.json")
...
func MigrateLegacyPlanSettingsFile(currentUserId string) {
	...
	settingsPath := filepath.Join(fs.HomeSophonDir, CurrentProjectId, CurrentPlanId, "settings.json")
	if _, err := os.Stat(settingsPath); err == nil {
		...
		v2Path := filepath.Join(fs.HomeSophonDir, CurrentProjectId, CurrentPlanId, "settings-v2.json")
