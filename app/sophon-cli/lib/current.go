...
func resolveProject(mustResolve, shouldCreate bool) {
	if fs.SophonDir == "" {
		var err error
		if shouldCreate {
			_, _, err = fs.FindOrCreateSophon()
		} else {
			fs.FindSophonDir()
		}

		if err != nil && mustResolve {
			term.OutputErrorAndExit("error finding or creating sophon: %v", err)
		}
	}

	if (fs.SophonDir == "" || fs.ProjectRoot == "") && mustResolve {
		fmt.Printf(
			"🤷‍♂️ No plans in current directory\nTry %s to create a plan or %s to see plans in nearby directories\n",
			color.New(color.Bold, term.ColorHiCyan).Sprint("sophon new"),
			color.New(color.Bold, term.ColorHiCyan).Sprint("sophon plans"))
		os.Exit(0)
	}
	...
	// check if projects-v2.json exists in SophonDir
	path := filepath.Join(fs.SophonDir, "projects-v2.json")
	_, err := os.Stat(path)
	...
	HomeCurrentProjectDir = filepath.Join(fs.HomeSophonDir, CurrentProjectId)
...
func mustInitProject(existingSettings *types.CurrentProjectSettingsByAccount) {
	...
	// write current-plans-v2.json to SophonHomeDir/[projectId]/current-plans-v2.json
	dir := filepath.Join(fs.HomeSophonDir, CurrentProjectId)
