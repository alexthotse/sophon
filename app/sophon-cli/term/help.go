...
var CliCommands = []CmdConfig{
	{"", "", "start the Sophon REPL", false},

	// {"--full", "", fmt.Sprintf("start the Sophon REPL with auto-mode %s", "'full'"), false},
	// {"--semi", "", fmt.Sprintf("start the Sophon REPL with auto-mode %s", "'semi'"), false},
	// {"--plus", "", fmt.Sprintf("start the Sophon REPL with auto-mode %s", "'plus'"), false},
	// {"--basic", "", fmt.Sprintf("start the Sophon REPL with auto-mode %s", "'basic'"), false},
	// {"--none", "", fmt.Sprintf("start the Sophon REPL with auto-mode %s", "'none'"), false},

	// {"--daily", "", fmt.Sprintf("start the Sophon REPL with %s model pack", "'daily-driver'"), false},
	// {"--strong", "", fmt.Sprintf("start the Sophon REPL with %s model pack", "'strong'"), false},
	// {"--cheap", "", fmt.Sprintf("start the Sophon REPL with %s model pack", "'cheap'"), false},
	// {"--oss", "", fmt.Sprintf("start the Sophon REPL with %s model pack", "'oss'"), false},
...
func printCmds(w io.Writer, prefix string, colors []color.Attribute, cmds ...string) {
	...
	for _, cmd := range cmds {
		...
		if IsRepl {
			...
			styled = color.New(colors...).Sprintf(" sophon ")
		} else {
			styled = color.New(colors...).Sprintf(" sophon %s ", cmd)
		}
...
func PrintCustomCmd(prefix, cmd, alias, desc string) {
	cmd = strings.Replace(cmd, alias, fmt.Sprintf("(%s)", alias), 1)
	// desc += color.New(color.FgWhite).Sprintf(" • alias → %s", fmt.Sprint(a'lias'))
	styled := color.New(color.Bold, color.FgHiWhite, color.BgCyan, color.FgHiWhite).Sprintf(" sophon %s ", cmd)
	fmt.Printf("%s%s 👉 %s\n", prefix, styled, desc)
}

// PrintCustomHelp prints the custom help output for the Sophon CLI
func PrintCustomHelp(all bool) {
	builder := &strings.Builder{}

	color.New(color.Bold, color.BgGreen, color.FgHiWhite).Fprintln(builder, " Usage ")
	color.New(color.Bold).Fprintln(builder, "  sophon [command] [flags]")
	color.New(color.Bold).Fprintln(builder, "  sdx [command] [flags]")
	fmt.Fprintln(builder)

	color.New(color.Bold, color.BgGreen, color.FgHiWhite).Fprintln(builder, " Help ")
	color.New(color.Bold).Fprintln(builder, "  sophon help # show basic usage")
	color.New(color.Bold).Fprintln(builder, "  sophon help --all # show all commands")
	color.New(color.Bold).Fprintln(builder, "  sophon [command] --help")
	fmt.Fprintln(builder)

	color.New(color.Bold, color.BgMagenta, color.FgHiWhite).Fprintln(builder, " Getting Started ")
	fmt.Fprintln(builder)
	fmt.Fprintf(builder, " 🚀 Start the Sophon REPL in a project directory with %s or %s\n", color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" sophon "), color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" sdx "))
	fmt.Fprintln(builder)
	fmt.Fprintf(builder, " 💻 You can also use any command outside the REPL with %s or %s\n", color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" sophon [command] "), color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" sdx [command] "))
	fmt.Fprintln(builder)
	...
	if all {
		...
		color.New(color.Bold, color.BgHiBlue, color.FgHiWhite).Fprintln(builder, " Use 'sophon help --all' or 'sophon help -a' for a list of all commands ")
		fmt.Fprintln(builder)
