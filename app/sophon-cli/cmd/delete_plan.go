...
func delAll() {
	fmt.Printf("%s\n\n", color.New(color.Bold, color.BgHiRed, color.FgHiWhite).Sprint(" ⚠️  WARNING "))
	fmt.Printf("You are about to delete %s plans in this project.\n", color.New(color.Bold, color.FgHiRed).Sprint("ALL"))
	fmt.Println("This action is permanent and cannot be undone.")
	fmt.Println()

	confirmed, err := term.ConfirmYesNo("Are you absolutely sure you want to proceed?")
	if err != nil {
		term.OutputErrorAndExit("Error getting confirmation: %v", err)
	}
	if !confirmed {
	...
	term.StartSpinner("")
	apiErr := api.Client.DeleteAllPlans(lib.CurrentProjectId)
	term.StopSpinner()

	if apiErr != nil {
		term.OutputErrorAndExit("Error deleting all plans: %v", apiErr)
	}
