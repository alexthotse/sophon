...
var replCmd = &cobra.Command{
	Use:   "repl",
	Short: "Start interactive Sophon REPL",
	Run:   runRepl,
}
...
func runRepl(cmd *cobra.Command, args []string) {
	...
	p = prompt.New(
		...
		prompt.WithTitle("Sophon "+version.Version),
...
func executor(in string, p *prompt.Prompt) {
	...
	// Handle sophon/sdx command prefix
	if strings.HasPrefix(lastLine, "sophon ") || strings.HasPrefix(lastLine, "sdx ") {
		fmt.Println()
		parts := strings.Fields(lastLine)
		if len(parts) > 1 {
			args := parts[1:] // Skip the "sophon" or "sdx" command
			_, err := lib.ExecSophonCommandWithParams(args, lib.ExecSophonCommandParams{
				SessionId: sessionId,
			})
			if err != nil {
	...
	if lib.CurrentReplState.Mode == lib.ReplModeTell {
		...
		output, err := lib.ExecSophonCommandWithParams(args, lib.ExecSophonCommandParams{
			SessionId: sessionId,
		})
		if err != nil {
...
func replWelcome(params replWelcomeParams) {
	...
	color.New(color.FgHiWhite, color.BgBlue, color.Bold).Print(" 👋 Welcome to Sophon ")
...
func handleRunCommand(args []string) error {
	...
	// Execute the command
	_, err := lib.ExecSophonCommand(cmdArgs)
	if err != nil {
...
func execWithInput(params execWithInputParams) execWithInputResult {
	...
	default:
		...
		if matchedCmd != "" {
			...
			_, err := lib.ExecSophonCommandWithParams(execArgs, lib.ExecSophonCommandParams{
				SessionId: sessionId,
			})
			if err != nil {
				color.New(term.ColorHiRed).Printf("Error executing command: %v\n", err)
